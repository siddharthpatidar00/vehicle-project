...
export class ApplyForLoanComponent {
  ...
  submitForm() {
    ...
    const payload: Loan = this.applyForLoanForm.value as Loan;

    this.loanService.createLoanInquiry(payload).subscribe({
      next: () => {
        this.toast.success('Loan Inquiry Submitted Successfully.');
        this.applyForLoanForm.reset();
        this.closeModal();
      },
      error: () => {
        this.toast.error('Failed to submit Loan Inquiry.');
      }
    });
  }
